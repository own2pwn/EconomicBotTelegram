...
bot.on('message', function(msg, match, reply) {
	if (msg.text === "/start") {
		...
		bot.sendMessage(msg.from.id, 'Сколько долларов ты хочешь перевести в рубли?', opts).then(function(sended) {
		...
		bot.sendMessage(msg.from.id, 'Сколько рублей ты хочешь перевести в доллары?', opts).then(function(sended) {
			var chatid = sended.chat.id;
			var messageid = sended.message_id;
		...
		bot.sendMessage(msg.from.id, 'Сколько долларов ты хочешь перевести в евро?', opts).then(function(sended) {
			var chatid = sended.chat.id;
			var messageid = sended.message_id;
		...
		bot.sendMessage(msg.from.id, 'Сколько евро ты хочешь перевести в доллары?', opts).then(function(sended) {
			var chatid = sended.chat.id;
			var messageid = sended.message_id;
			bot.onReplyToMessage(chatid, messageid, function(message) {
				if (/[0-9]/.test(message.text)) {
